...
from modules import Embedding, PCNN, Attention
...
class APCNN(nn.Module):
    ...
    def forward(self, sentence, position1, position2, entity_pos, entity_pair):
        ...
        if bag_size == 1:
            ...
            self.attention_weight = self.attention(pcnn_feature, entity_embedding[0], entity_embedding[1])
            if self.slice_rate is None:
                weighted_pcnn_feature = torch.mul(pcnn_feature, self.attention_weight.unsqueeze(-1))
                bag_pcnn_feature = torch.sum(weighted_pcnn_feature, dim=0, keepdim=True)
                output = self.linear(bag_pcnn_feature)
            else:
                thres = self.slice_rate * float(torch.max(self.attention_weight))
                mask = torch.ge(self.attention_weight, thres)
                masked_pcnn_feature = torch.masked_select(pcnn_feature, mask.unsqueeze(-1)).view(-1, pcnn_feature.shape[1])
                masked_weight = torch.masked_select(self.attention_weight, mask)
                masked_weight /= torch.sum(masked_weight)
                weighted_masked_feature = torch.mul(masked_pcnn_feature, masked_weight.unsqueeze(-1))
...
def predict(model, relations, counts, sents, poss, eposs, epair, seq_len):
    ...
    with torch.no_grad():
        ...
        for bag_idx, instance_relation in enumerate(relations):
            ...
            result = model(ins_sentence, ins_pos1, ins_pos2, ins_epos, ins_epair) # (1 * class_num)
...
def train(snapshot_path, prfile_path, verbose=True, logger=None):
    parser = argparse.ArgumentParser(description="Train APCNN model.")
    ...
    parser.add_argument("--slice_rate", type=float, default=0.4, help="ATS attention threshold.")
    ...
    logger("Load model.")
    slice_rate = None if args.slice_rate == 0 else args.slice_rate
    model = APCNN(args.class_num, word_embedding_matrix, position1_embedding_matrix, position2_embedding_matrix, \
        args.filters, (args.kernel_size, word_dim + 2 * pos_dim), padding=(int(args.kernel_size/2), 0), slice_rate=slice_rate).to(device)
