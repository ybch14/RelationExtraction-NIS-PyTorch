...
class PCNN_ATT(nn.Module):
    ...
    def forward(self, sentence, position1, position2, entity_pos, label=None):
        ...
        if label is not None: # train mode
            if pcnn_feature.shape[0] == 1:
                dropout_output = self.dropout(pcnn_feature)
                output = self.linear(dropout_output)
                return output
            else:
                attention_feature = torch.mm(pcnn_feature, self.linear.weight.t())[:, label]
                self.attention = F.softmax(attention_feature, dim=0)
                weighted_pcnn_feature = torch.mul(pcnn_feature, self.attention.unsqueeze(-1))
            ...
            if pcnn_feature.shape[0] == 1:
                ...
                attention_feature = torch.mm(pcnn_feature, self.linear.weight.t())
                attention_feature = F.softmax(attention_feature, dim=0)
                max_prob = torch.zeros(self.class_num)
                for i in range(self.class_num):
                    attention = attention_feature[:, i]
                    weighted_pcnn_feature = torch.mul(pcnn_feature, attention.unsqueeze(-1))
                    bag_pcnn_feature = torch.sum(weighted_pcnn_feature, dim=0, keepdim=True)
                    bag_pcnn_feature = self.dropout(bag_pcnn_feature)
                    bag_feature = self.linear(bag_pcnn_feature).squeeze()
                    bag_prob = F.softmax(bag_feature, dim=0).detach()
...
def train(snapshot_path, prfile_path, verbose=True, logger=None):
    parser = argparse.ArgumentParser(description="Train PCNN+ATT model.")
    ...
    logger("Load model.")
    model = PCNN_ATT(args.class_num, word_embedding_matrix, position1_embedding_matrix, position2_embedding_matrix, \
